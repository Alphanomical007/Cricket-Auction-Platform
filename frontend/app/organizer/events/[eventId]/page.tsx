...
function TeamsTab({
  ...
  onUploadTeamLogo: (teamId: number, file: File) => void;
  editable: boolean;
}) {
            ...
            return (
              <div key={team.id} className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
                ...
                >
                  {/* Team logo thumbnail */}
                  {team.logo && (
                    <div className="w-10 h-10 rounded-full overflow-hidden bg-white/20 border border-white/30 shrink-0 mr-3">
                        ...
                        {editable && (
                          <>
                            ...
                            <label
                              ...
                              className="text-xs px-2 py-1 rounded-lg hover:bg-white/20 transition-colors cursor-pointer"
                            ...
                            >
                              ...
                              <input
                                ...
                                onChange={(e) => {
                                  ...
                                  if (file) onUploadTeamLogo(team.id, file);
                                }}
                              />
                            </label>
                            <button
...
export default function OrganizerEventDetailPage() {
  ...
  const [uploadingTeamLogoId, setUploadingTeamLogoId] = useState<number | null>(null);
  ...
  const uploadTeamLogo = async (teamId: number, file: File) => {
    setUploadingTeamLogoId(teamId);
    try {
      const formData = new FormData();
      formData.append("file", file);
      await api.post(`/organizer/events/${eid}/teams/${teamId}/logo`, formData, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      await fetchAll();
      showToast("ok", "Team logo uploaded!");
    } catch (err: unknown) {
      const msg = (err as { response?: { data?: { detail?: string } } })?.response?.data?.detail;
      showToast("err", msg || "Failed to upload team logo");
    } finally {
          ...
          <div className="lg:col-span-3">
            ...
            {tab === "teams" && (
              <TeamsTab
                ...
                onUploadTeamLogo={uploadTeamLogo}
                editable={isDraft}
              />
            )}
