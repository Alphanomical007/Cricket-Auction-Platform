...
async def upload_event_logo(
    ...
    """Upload a logo for the event (saves locally if S3 not configured)."""
    event = await _get_event_for_organizer(event_id, current_user, db)
...
@router.post("/events/{event_id}/teams/{team_id}/logo", response_model=TeamOut)
async def upload_team_logo(
    ...
    """Upload a logo for a team (saves locally if S3 not configured)."""
    await _get_event_for_organizer(event_id, current_user, db)
    ...
    url = upload_file(content, key_prefix=f"team-logos/{team_id}", content_type=file.content_type)
    if not url:
