...
from app.schemas.user import UserOut, AssignRolePayload, UserBulkCreate
...
async def list_users(
    ...
    result = await db.execute(
        select(User).where(User.deleted_at.is_(None), User.is_active == True).order_by(User.created_at.desc())
    )
...
async def bulk_create_users(
    payload: UserBulkCreate,
    current_user: User = Depends(require_role(ROLE_ADMIN)),
    db: AsyncSession = Depends(get_db),
):
    # Fetch existing emails
    existing_result = await db.execute(select(User.email))
    existing_emails = {e for e in existing_result.scalars().all()}
    ...
    for rec in payload.users:
        ...
        user = User(
            ...
            phone=rec.phone,
            batting_rating=rec.batting_rating,
            bowling_rating=rec.bowling_rating,
            fielding_rating=rec.fielding_rating,
            profile_photo=rec.profile_photo,
        )
        db.add(user)
        new_users.append(rec.email)
        existing_emails.add(rec.email)
    
    if new_users:
        await db.commit()

    return {"created": len(new_users), "skipped": skipped, "message": "Bulk import completed"}
